...
public class SingleClassExample {
	...
	public static void main(String[] args) throws IOException {
		...
		// We need to create legui context which shared by renderer and event
		// processor.
		...
		// These callbacks will be used in initialization of system event
		// processor
		// (will be added callbacks which will push system events to event queue
		// and after that processed by SystemEventProcessor)
		CallbackKeeper keeper = new DefaultCallbackKeeper();

		// register callbacks for window. Note: all previously binded callbacks
		// will be unbinded.
		...
		// if we want to create some callbacks for system events you should
		// create and put them to keeper
		//
		// Wrong:
		...
		// Event processor for system events. System events should be processed
		// and translated to gui events.
		...
		while (running) {
			...
			// Before rendering we need to update context with window size and
			// window framebuffer size
			// {
			// int[] windowWidth = {0}, windowHeight = {0};
			// GLFW.glfwGetWindowSize(window, windowWidth, windowHeight);
			// int[] frameBufferWidth = {0}, frameBufferHeight = {0};
			// GLFW.glfwGetFramebufferSize(window, frameBufferWidth,
			// frameBufferHeight);
			// int[] xpos = {0}, ypos = {0};
	...
	private static List<Component> generateOnFly() {
		...
		Label nameLabel = new Label(180, 120, 200, 20);
		nameLabel.getTextState().setText("Kmety");
		nameLabel.getTextState().setTextColor(ColorConstants.blue());
